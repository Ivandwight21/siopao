...
$flash = consume_flash();
$prefillEmail = sanitize_email($_GET['email'] ?? null);
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>SioPao - Reset Password</title>
...
<body>
    <div class="login-container">
        <div class="left-panel">
            ...
            <p>Choose a password that is unique and secure so you can keep enjoying freshly steamed SioPao without worry.</p>
        </div>
        <div class="right-panel">
            <div class="login-header">
                <h1>Set new password</h1>
                <p>Enter and confirm your new password below. Strong passwords protect your account and your favorite orders.</p>
            </div>
            <?php if ($flash): ?>
            ...
            <?php endif; ?>
            <form id="resetForm" method="POST" action="auth/verify_otp.php" novalidate>
                <div class="form-group">
                    <label for="email">Email address</label>
                    <div class="input-with-icon">
                        <i class="fas fa-envelope"></i>
                        <input type="email" id="email" name="email" class="form-control" value="<?php echo htmlspecialchars($prefillEmail ?? ''); ?>" placeholder="you@example.com" required>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="input-with-icon">
                        ...
                        <input type="text" id="otp" name="otp" class="form-control" placeholder="Enter 6-digit code" pattern="\d{6}" required>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="input-with-icon">
                        ...
                        <input type="password" id="newPassword" name="new_password" class="form-control" placeholder="Enter a strong password" required>
                    </div>
                    <p class="password-hint">Use at least 8 characters, including an uppercase letter and a number.</p>
                </div>
                <div class="form-group">
                    <label for="confirmPassword">Confirm password</label>
                    <div class="input-with-icon">
                        <i class="fas fa-lock"></i>
                        <input type="password" id="confirmPassword" name="confirm_password" class="form-control" placeholder="Re-enter your new password" required>
                    </div>
                </div>
    ...
    <script>
        ...
        function meetsStrength(password) {
        ...
        resetForm.addEventListener('submit', function(event) {
            ...
            if (!meetsStrength(newPassword)) {
                event.preventDefault();
                alert('Please follow the password guidelines before proceeding.');
                return;
            }

            if (newPassword !== confirmPassword) {
                event.preventDefault();
                alert('The passwords do not match. Double-check and try again.');
                return;
            }
        });
    </script>
